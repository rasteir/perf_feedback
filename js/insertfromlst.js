$(document).ready(function () {
	$("#submit").click(function () {
		$.ajax({
			type: "POST",
			url: "php/insert.php",
			data: "peer_mail=" + ($("#peer_mail option:selected").val()).toLowerCase() + "&strenghts-comments=" + $("#strenghts-comments").val() + "&improvement-comments=" + $("#improvement-comments").val() + "&free-comments=" + $("#free-comments").val(),
		...
		$.ajax({
			...
			data: "peer_mail=" + ($("#peer_mail option:selected").val()).toLowerCase() + "&user_mail=" + $("#user_mail").val(),
			success: function (msg) {
				if (msg == 1) {
				}
				else {
